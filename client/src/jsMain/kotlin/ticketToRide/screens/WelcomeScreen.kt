...
data class WelcomeScreenState(
    ...
    val playerColor: PlayerColor? = PlayerColor.entries.first(),
...
val WelcomeScreen = FC<WelcomeScreenProps> { props ->
    ...
    val appStrings = useMemo(props.locale) { AppStrings(props.locale) }
        ...
        setState(
            state.copy(
                ...
                playerColor = if (state.otherPlayers.map { it.color }.contains(state.playerColor))
                    state.otherPlayers.getNotUsedColors(state.playerName).firstOrNull()
                else
                    state.playerColor
            )
        )
...
val radioButtonThemes = PlayerColor.entries.associateWith {
    createTheme(
        jso {
            palette = jso {
...
private fun ChildrenBuilder.playerColor(state: WelcomeScreenState, setState: StateSetter<WelcomeScreenState>, str: WelcomeScreenStrings) {
    RadioGroup {
        ...
        state.otherPlayers.getNotUsedColors(state.playerName).forEach {
            ThemeProvider {
...
private fun ChildrenBuilder.joinAsObserver(state: WelcomeScreenState, setState: StateSetter<WelcomeScreenState>, str: WelcomeScreenStrings) {
    Typography {
        ...
        div {
            ...
            state.otherPlayers.forEachIndexed { ix, player ->
                span {
                    css {
                        width = 8.px
                        height = 8.px
                ...
                val separator =
                    if (ix == state.otherPlayers.size - 2) " and" else if (ix < state.otherPlayers.size - 2) "," else ""
                span {
                    +"${player.name.value}$separator "
                }
            }
            val verb = if (state.otherPlayers.size == 1) "is" else "are"
...
private fun proceed(gameId: GameId?, props: WelcomeScreenProps, state: WelcomeScreenState, setState: StateSetter<WelcomeScreenState>, str: WelcomeScreenStrings) {
    ...
    when {
        gameId == null -> {
            val customMap = state.customMap?.map
            for (entry in (customMap ?: defaultMap).segments.groupingBy { it.color }
