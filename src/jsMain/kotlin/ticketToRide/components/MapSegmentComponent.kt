...
class MapSegmentComponent: RComponent<MapSegmentProps, RState>() {
    ...
    override fun RBuilder.render() {
        ...
        line = Polyline(jsObject {
            ...
            icons =
                if (occupiedBy == null) arrayOf(segmentSplitIcon(props.color, props.points, false))
                else occupiedSegmentIcons(occupiedBy.color, props.points, props.mapZoom)
        })
    }

    private fun occupiedSegmentIcons(color: Color, points: Int, mapZoom: Int) : Array<IconSequence> {
        val iconsList = mutableListOf(
            dashedLikeIcon(color.rgb),
            segmentSplitIcon(color, points, true),
            occupiedIcon(points, color.rgb)
        )
        if (mapZoom > 5) iconsList += carIcon(color.rgb)
        return iconsList.toTypedArray()
    }

    private fun segmentSplitIcon(color: Color?, points: Int, occupied: Boolean) = jsObject<IconSequence> {
        icon = jsObject {
            path = if (occupied) "M 1,1 -1,-1 M 1,-1 -1,1" else "M 1,0 -1,0"
            strokeOpacity = 1
            strokeColor = if (color != Color.BLACK) "#000000" else "#FFFFFF"
            scale = 3
        }
        offset = "0px"
