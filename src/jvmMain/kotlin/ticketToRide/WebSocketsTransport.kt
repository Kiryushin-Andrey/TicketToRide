    ...
    routing {
        webSocket(path) {
            ...
            when (val outcome = establishConnection(gameId, rootScope, formatter, redis)) {
                        ...
                        .collect { msg ->
                            kotlin.runCatching { formatter.deserialize(msg, Request.serializer()) }.fold(
                                ...
                                onFailure = { e ->
                                    ...
                                    game.process(LeaveGameRequest, player)
                                })
                        }
                ...
                is ConnectionOutcome.ObserveSuccess -> outcome.apply {
                ...
                // something went wrong in connection handshake
...
private suspend fun WebSocketSession.establishConnection(
    ...
    // first tell the client what serialization format to use
    send(formatter.id)
